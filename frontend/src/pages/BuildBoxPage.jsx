...
export default function BuildBoxPage() {
  ...
  const handleSaveTemplate = () => {
    ...
    setTimeout(() => { setSaving(false); toast.success(`"${boxName}" saved as a template!`); }, 400);
  };
        ...
        {savedBoxes.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Your Saved Boxes</h2>
            </div>
              ...
              {savedBoxes.map(box => {
                ...
                return (
                  <button key={box.id} onClick={() => handleLoadTemplate(box)}
                    ...
                    <p className="mt-2.5 text-xs font-medium text-brand-500 group-hover:underline">Tap to reload →</p>
                  </button>
                );
              })}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {loading ? (
              ...
              <div className="card p-10 text-center text-gray-400">No meals found</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filtered.map(meal => {
                      ...
                      <div className="p-3">
                        ...
                        {isOutOfStock ? (
                          ...
                          <button onClick={() => addMeal(meal._id)} disabled={boxFull}
                            ...
                            <Plus className="w-3.5 h-3.5" /> {boxFull ? 'Box Full' : 'Add'}
                          </button>
                        ) : (
                          <div className="flex items-center justify-between bg-gray-50 rounded-xl px-1 py-1">
                            ...
                            <div className="text-center">
                              ...
                              <p className="text-xs text-gray-400 leading-none">{qty === MAX_QTY_PER_MEAL ? 'max' : `of ${MAX_QTY_PER_MEAL}`}</p>
                            </div>
                    ...
                    return (
                      <div key={meal._id} className={`card overflow-hidden ring-1 bg-brand-50/30 ${isAdded ? 'ring-brand-500' : 'ring-brand-200'}`}>
                        ...
                        <div className="p-3">
                          ...
                          {!isAdded ? (
                            <button onClick={() => addMeal(meal._id)} className="w-full py-1.5 rounded-xl text-xs font-medium bg-brand-50 text-brand-600 border border-brand-200 hover:bg-brand-500 hover:text-white transition-all">+ Add</button>
                          ) : (
                            <div className="flex items-center justify-between bg-gray-50 rounded-xl px-1 py-1">
          ...
          <div className="lg:col-span-1">
            <div className="card p-6 sticky top-24">
              ...
              <button onClick={handleSaveTemplate} disabled={saving || mealCount === 0}
                ...
                <Bookmark className="w-4 h-4" /> {saving ? 'Saving…' : 'Save as Template'}
              </button>
              ...
              <button onClick={handleAddToCart} disabled={adding || mealCount === 0} className="btn-primary w-full flex items-center justify-center gap-2 mb-3">
                <ShoppingCart className="w-4 h-4" /> {adding ? 'Creating Box...' : 'Add to Cart'}
              </button>
