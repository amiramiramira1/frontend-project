...
export default function CheckoutPage() {
  ...
  const validate = () => {
    ...
    if (!form.street) newErrors.street = t('checkout.streetReq');
    if (form.zip && !/^\d{5}$/.test(form.zip)) newErrors.zip = t('checkout.zipError');
    if (!/^01[0125][0-9]{8}$/.test(form.phone)) newErrors.phone = t('checkout.phoneError');
    if (!form.timeSlot) newErrors.timeSlot = t('checkout.timeReq');
    if (!form.deliveryDate) newErrors.deliveryDate = t('checkout.dateReq');
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || i18next.t('msg.orderFailed'));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-100">
        <div className="page-container py-8">
          <h1 className="font-display text-4xl font-bold text-gray-900">{t('checkout.title')}</h1>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="card p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <input value={form.street} onChange={e => { setForm(p => ({ ...p, street: e.target.value })); setErrors(p => ({ ...p, street: '' })); }} className={`input-field ${errors.street ? 'border-red-500 focus:ring-red-400' : ''}`} placeholder={t('checkout.streetPh')} />
                    {errors.street && <p className="text-red-500 text-xs mt-1">{errors.street}</p>}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('checkout.cityLabel')}</label>
                    <select required value={form.city} onChange={e => setForm(p => ({ ...p, city: e.target.value }))} className="input-field">
                      {cities.map(c => <option key={c}>{c}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('checkout.zipLabel')}</label>
                    <input value={form.zip} onChange={e => { setForm(p => ({ ...p, zip: e.target.value })); setErrors(p => ({ ...p, zip: '' })); }} className={`input-field ${errors.zip ? 'border-red-500 focus:ring-red-400' : ''}`} placeholder="11511" />
                    {errors.zip && <p className="text-red-500 text-xs mt-1">{errors.zip}</p>}
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('checkout.phoneLabel')}</label>
                    <div className="relative">
                      <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                      <input value={form.phone} onChange={e => { setForm(p => ({ ...p, phone: e.target.value })); setErrors(p => ({ ...p, phone: '' })); }} className={`input-field pl-11 ${errors.phone ? 'border-red-500 focus:ring-red-400' : ''}`} placeholder="01012345678" />
                    </div>
              ...
              <div className="card p-6">
                <h2 className="font-display text-xl font-bold mb-5 flex items-center gap-2">
                  <Truck className="w-5 h-5 text-brand-500" /> {t('checkout.schedTitle')}
                </h2>
                <div className="space-y-4">
                  <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('checkout.dateLabel')}</label>
                  <DatePicker
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('checkout.timeLabel')}</label>
              ...
              <div className="card p-6">
                <h2 className="font-display text-xl font-bold mb-5 flex items-center gap-2">
                  <CreditCard className="w-5 h-5 text-brand-500" /> {t('checkout.payTitle')}
                </h2>
                <div className="flex items-center gap-4 p-4 bg-brand-50 border-2 border-brand-500 rounded-xl">
                  ...
                  <div>
                    <div className="font-semibold text-brand-900">{t('checkout.cashTitle')}</div>
                    <div className="text-sm text-brand-700">{t('checkout.cashDesc')}</div>
                  </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-700 flex gap-3">
                ...
                <div>
                  <strong>{t('checkout.estDelivery')}</strong> {t('checkout.estNote')}
                </div>
              </div>
            </div>
