...
export default function BoxDetailPage() {
  ...
  const handleSubmitReview = () => {
    ...
    if (reviews.some(r => r.name === user?.name)) { toast.error('You have already reviewed this box!'); return; }
      ...
      <div className="relative h-72 md:h-96">
        ...
        <div className="absolute bottom-6 left-6 right-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="badge bg-brand-500 text-white capitalize">{box.dietType}</span>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="card p-6">
              ...
              <div className="flex gap-6 text-sm text-gray-500">
                {/* meals.length replaces mealsCount */}
                <span className="flex items-center gap-1.5"><ChefHat className="w-4 h-4 text-brand-500" /> {box.meals?.length || 0} Meals</span>
                <span className="flex items-center gap-1.5"><Users className="w-4 h-4 text-brand-500" /> Serves 1–6</span>
              </div>
            </div>

            {/* Meals — now box.meals array (populated by backend) */}
            <div>
            ...
            {/* Allergens — flatmapped from meals */}
            {allergens.length > 0 && (
            ...
            {/* Reviews (still client-side until API is built) */}
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowForm(prev => !prev)} className="btn-primary flex items-center gap-1.5 text-sm px-3 py-2">
                  <PenLine className="w-4 h-4" /> Review
                </button>
              </div>
              ...
              {showForm && (
                <div className="card p-6 mb-6 border-2 border-brand-200 bg-brand-50/30">
                  ...
                  <button onClick={handleSubmitReview} className="btn-primary w-full py-3">Submit Review</button>
                </div>
              )}
              ...
              {reviews.length === 0 && !showForm && (
                <div className="card p-10 text-center">
                  ...
                  <p className="text-gray-500 text-sm mb-4">Be the first to review this box!</p>
                  <button onClick={() => setShowForm(true)} className="btn-primary px-6">Write the First Review</button>
                </div>
              )}
            ...
            <div className="card p-6 sticky top-24">
              ...
              <div className="bg-gray-50 rounded-xl p-4 mb-4">
                <div className="flex justify-between text-sm text-gray-500 mb-1">
                  <span>{box.meals?.length || 0} meals × {servings} {servings === 1 ? 'person' : 'people'}</span>
                  <span>{currentPricing?.pricePerServing?.toFixed(0)} EGP/meal</span>
                </div>
